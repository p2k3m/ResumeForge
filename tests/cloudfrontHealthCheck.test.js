...
describe('checkCloudfrontHealth', () => {
  ...
  test('verifies a root CloudFront distribution', async () => {
    const origin = 'https://d3r7s5t9uvwx2.cloudfront.net';
  ...
  test('verifies a distribution with a stage path', async () => {
    const originWithStage = 'https://d3r7s5t9uvwx2.cloudfront.net/prod';
    const pool = mockAgent.get('https://d3r7s5t9uvwx2.cloudfront.net');
    pool
    ...
    expect(result).toMatchObject({
      ok: true,
      url: 'https://d3r7s5t9uvwx2.cloudfront.net/prod/healthz',
    });
  });
...
describe('buildHealthCheckUrl', () => {
  test('normalizes root URL', () => {
    const url = buildHealthCheckUrl('https://d3r7s5t9uvwx2.cloudfront.net');
    expect(url.toString()).toBe('https://d3r7s5t9uvwx2.cloudfront.net/healthz');
  });
  ...
  test('normalizes URL with stage path', () => {
    const url = buildHealthCheckUrl('https://d3r7s5t9uvwx2.cloudfront.net/prod');
    expect(url.toString()).toBe('https://d3r7s5t9uvwx2.cloudfront.net/prod/healthz');
  });
});
