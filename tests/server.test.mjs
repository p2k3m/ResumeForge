...
describe.skip('static asset fallbacks', () => {
  ...
  test('serves the index asset alias when the manifest contains prefixed S3 keys', async () => {
    const cssAliasName = 'index-prefixed.css';
    const cssBody = '.prefixed { color: #0c0; }';
    ...
    mockS3Send
      .mockImplementationOnce((command) => {
        ...
        return Promise.resolve({
          Body: Readable.from([
            JSON.stringify({
              hashedIndexAssets: [`static/client/prod/latest/assets/${cssAliasName}`],
            }),
          ]),
          ContentType: 'application/json',
        });
      ...
      .mockImplementationOnce((command) => {
        expect(command.__type).toBe('GetObjectCommand');
        expect(command.input.Key).toBe(`static/client/prod/latest/assets/${cssAliasName}`);
        return Promise.resolve({
          Body: Readable.from([cssBody]),
          ContentType: 'text/css',
          CacheControl: 'public, max-age=60',
...
describe('/api/generate-enhanced-docs', () => {
  ...
  test('refreshes session artifacts when job description changes for a new enhancement', async () => {
    const existingChangeLog = {
      ...
      entries: [
        {
          id: 'existing-entry',
          title: 'Old JD change',
          detail: 'Outdated improvement detail',
        },
      ],
      dismissedEntries: [
        {
      ...
      coverLetters: {
        entries: [
          {
            id: 'cover_letter1',
            originalText: 'Original cover letter',
    ...
    const oldJobDescription = 'Drive reliability initiatives and lead engineering teams.';
    const newJobDescription =
      'Lead platform migrations and scale operations with cross-functional partners.';
    ...
    const response = await request(app)
      ...
      .send({
        ...
        originalResumeText: 'Alex Candidate\nExperience\n- Built resilient systems.',
        jobDescriptionText: newJobDescription,
        jobSkills: ['Leadership'],
        resumeSkills: ['Leadership'],
        enhancementIntent: 'new',
      });
    ...
    expect(storedPayload.coverLetters).toBeTruthy();
