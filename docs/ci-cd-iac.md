...
* **GitHub Actions deployment workflow** – `.github/workflows/deploy.yml` performs manual (`workflow_dispatch`) deployments. The workflow authenticates with AWS using the repository's access key and secret, rebuilds Lambda and client assets, deploys the SAM stack, and issues a CloudFront invalidation. Passing the resolved distribution domain directly into `npm run verify:cloudfront` keeps CDN health checks inside the pipeline. Inputs allow operators to skip the pre-deploy tests during emergency fixes or temporarily ignore CloudFront verification errors while an incident is mitigated.
