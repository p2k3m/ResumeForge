...
All workflows support both GitHub OIDC role assumption **and** traditional access key pairs. When `AWS_ROLE_TO_ASSUME` is set the jobs request temporary credentials via OIDC; otherwise they fall back to `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` (with an optional `AWS_SESSION_TOKEN`). This dual mode ensures static asset publishing and deployments keep working even when the repository only has long-lived IAM user credentials available.
