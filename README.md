...
## User flow
1. **Upload & validation.** Candidates drop a CV (PDF, DOC, or DOCX) into the React portal. The backend immediately uploads the original file to S3 under `resume-forge-data/<candidate_name>/cv/<date>/…`, creates a DynamoDB record with hashed personal data (IP, location, device, LinkedIn, Credly), and records the detected file type. If heuristics or Gemini determine the document is not a CV, the API responds with a descriptive error such as “You have uploaded a presentation document, please upload a correct CV.” so the UI can prompt for a proper résumé.
2. **Job description validation.** The service fetches the provided job-description URL. When the request is blocked (login walls, 4xx/5xx responses), the API returns `JOB_DESCRIPTION_FETCH_FAILED` with a `manualInputRequired` flag so the frontend immediately surfaces a text area instructing, “Unable to fetch JD from this URL. Please paste full job description below.”
3. **ATS analysis.** Once the CV and JD are available, the Lambda analyses both and produces the ATS score dashboard (Layout & Searchability, ATS Readability, Impact, Crispness, Other Quality Metrics) plus JD-to-CV alignment: designation deltas, experience gaps, skill/task matches, certifications, and highlights. The UI also shows the overall ATS score and the calculated selection probability with actionable insights (missing skills, designation mismatch, missing certifications, highlights added, etc.).
4. **Improvement & enhancement.** Each category exposes an **Improve** button powered by AI to rewrite summaries, blend missing skills, realign designations, adjust experience, and surface certifications or highlights. Candidates can accept or reject individual suggestions. **Improve All** generates ATS-perfect CV PDFs (2025 design) and an accompanying tailored cover letter, with download links presented alongside the change log explaining what was added.
5. **Download & prepare.** After enhancements, the portal lists the generated CVs and cover letters for download and reiterates that the “chances of selection have increased” so candidates can prepare for interviews with the highlighted skills.

Old S3 sessions are automatically purged after 30 days to honour GDPR commitments, and every interaction is handled via the serverless Express Lambda/API Gateway stack while the React frontend is delivered through CloudFront.
