...
const SECTION_SYNONYMS = Object.freeze(
  Object.fromEntries(
    Object.entries(RAW_SECTION_SYNONYMS).map(([key, values]) => {
      const normalized = new Set();
      values.forEach((value) => {
        const resolved = normalizeHeadingKey(value);
        if (resolved) normalized.add(resolved);
      });
      normalized.add(normalizeHeadingKey(key));
      return [key, Array.from(normalized)];
    })
  )
);
...
function takeSection(sectionMap, usedSections, key) {
  const variants = SECTION_SYNONYMS[key] || [key];
  for (const variant of variants) {
    const section = sectionMap.get(variant);
...
export async function render2025Template({
  ...
  const take = (key) => takeSection(sectionMap, usedSections, key);

  const contactSection = take('contact');
  const summarySection = take('summary');
  const experienceSection = take('experience');
  const educationSection = take('education');
  const projectsSection = take('projects');
  const skillsSection = take('skills');
  const certificationsSection = take('certifications');
  const languagesSection = take('languages');
  ...
  const entryOptions = { enhancementTokenMap };

  const summaryEntries = extractEntries(summarySection, entryOptions);
  const experienceEntries = extractEntries(experienceSection, entryOptions);
  const educationEntries = extractEntries(educationSection, entryOptions);
  const projectEntries = extractEntries(projectsSection, entryOptions);
  const skillsEntries = extractEntries(skillsSection, entryOptions);
  const languageEntries = extractEntries(languagesSection, entryOptions);
  const contactEntries = extractEntries(contactSection, entryOptions);
  const certificationEntries = extractEntries(certificationsSection, entryOptions);

  if (options.project && (!projectEntries || !projectEntries.length)) {
    projectEntries.push({ text: options.project, bullet: true });
  }

  const contactHeading = contactSection?.heading || 'Contact';
  const summaryHeading = summarySection?.heading || 'Summary';
  const experienceHeading = experienceSection?.heading || 'Experience';
  const educationHeading = educationSection?.heading || 'Education';
  const projectsHeading = projectsSection?.heading || 'Projects';
  const skillsHeading = skillsSection?.heading || 'Skills';
  const certificationsHeading =
    certificationsSection?.heading || 'Certifications';
  const languagesHeading = languagesSection?.heading || 'Languages';
  ...
  drawSidebarHeading(sidebarContext, contactHeading);
  ...
  drawSidebarHeading(sidebarContext, skillsHeading);
  ...
  if (certificationEntries.length) {
    sidebarContext.y -= sidebarLineGap;
    drawSidebarHeading(sidebarContext, certificationsHeading);
  ...
  sidebarContext.y -= sidebarLineGap;
  drawSidebarHeading(sidebarContext, languagesHeading);
  if (languages.length) {
  ...
  drawRightHeading(rightContext, educationHeading);
  ...
  drawRightHeading(rightContext, skillsHeading);
  drawRightEntries(rightContext, skillTextEntries, { allowBullets: false });

  const additionalSections = sections.filter(
    (section) => section && !usedSections.has(section)
  );

  for (const section of additionalSections) {
    const entries = extractEntries(section, entryOptions);
    if (!entries.length) continue;
    const heading = section.heading || 'Additional Information';
    const allowBullets = entries.some((entry) => entry.bullet);
    drawRightHeading(rightContext, heading);
    drawRightEntries(rightContext, entries, { allowBullets });
    rightContext.y -= sectionGap / 2;
  }
