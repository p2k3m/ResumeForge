...
const STATIC_ASSET_PATH_PATTERN = /^\/(?:assets|fonts|images|cover-templates)(?:\b|\/)/i;
...
function parsePossibleJson(body) {
...
function countArrayEntries(candidate) {
  if (!candidate) {
    return 0;
  }
  if (Array.isArray(candidate)) {
    return candidate.filter((value) => value !== null && value !== undefined).length;
  }
  return 0;
}

function inferSessionCount(payload) {
  if (!payload || typeof payload !== 'object') {
    return 0;
  }
  ...
  const numericCandidates = [
    ...
    payload.sessionLogs?.length,
  ].filter((value) => Number.isFinite(value) && value > 0);

  let count = numericCandidates.length
    ? Math.max(...numericCandidates.map((value) => Number(value)))
    : 0;
  ...
  if (Array.isArray(payload.sessionIds)) {
    count = Math.max(count, payload.sessionIds.length);
  }
  if (Array.isArray(payload.sessions)) {
    count = Math.max(count, payload.sessions.length);
  }
  if (Array.isArray(payload.sessionLogs)) {
    count = Math.max(count, payload.sessionLogs.length);
  }
  return count;
}

const DOWNLOAD_ARRAY_KEYS = [
  'urls',
  'downloads',
  'downloadUrls',
  'download_logs',
...
const DOWNLOAD_COUNT_KEYS = ['downloadCount', 'totalDownloads', 'downloadTotal'];

function inferDownloadCount(payload) {
  if (!payload || typeof payload !== 'object') {
    return 0;
  }
  ...
  for (const key of DOWNLOAD_COUNT_KEYS) {
  ...
  for (const key of DOWNLOAD_ARRAY_KEYS) {
    if (Array.isArray(payload[key])) {
      count = Math.max(count, countArrayEntries(payload[key]));
    }
  }
...
function resolveHttpPath(event = {}, requestMetadata = {}) {
  if (typeof requestMetadata?.path === 'string' && requestMetadata.path) {
    return requestMetadata.path;
  }
  if (typeof event?.rawPath === 'string' && event.rawPath) {
...
function resolveHttpMethod(event = {}, requestMetadata = {}) {
  if (typeof requestMetadata?.method === 'string' && requestMetadata.method) {
    return requestMetadata.method;
  }
...
function resolveHttpPathCategory(path = '') {
  ...
  if (path.startsWith('/cover-templates')) {
    return '/cover-templates';
  }
  return path.split('?')[0] || 'unknown';
}

function describeHttpResponse(result, { event, requestMetadata, durationMs } = {}) {
  if (!result || typeof result !== 'object') {
    return null;
  }

  const headers = normalizeHeaderMap(result.headers);
  const statusCode = Number.isInteger(result.statusCode)
    ? Number(result.statusCode)
    : undefined;
  const method = resolveHttpMethod(event, requestMetadata);
  const path = resolveHttpPath(event, requestMetadata);
  ...
  const isJsonContent =
    typeof contentTypeHeader === 'string' && /json/i.test(contentTypeHeader);
  if (!result.isBase64Encoded && typeof result.body === 'string' && result.body) {
    parsedBody = isJsonContent ? parsePossibleJson(result.body) : parsePossibleJson(result.body);
  }

  const sessionCount = inferSessionCount(parsedBody);
  const downloadCount = inferDownloadCount(parsedBody);
  const pathCategory = resolveHttpPathCategory(path);
  const isAssetRequest = STATIC_ASSET_PATH_PATTERN.test(path);
  ...
  return {
    ...
    sessionCount: Number.isFinite(sessionCount) && sessionCount > 0 ? sessionCount : 0,
...
function compactLogObject(payload = {}) {
  if (!payload || typeof payload !== 'object') {
    return {};
  }
...
function buildAdditionalMetricsFromHttpSummary(summary, baseDimensions, timestamp) {
  if (!summary) {
    return [];
  }

  const createDimensions = (extra = {}) => {
    const cloned = baseDimensions.map((entry) => ({ ...entry }));
    if (summary.pathCategory) {
      cloned.push({ Name: 'Path', Value: summary.pathCategory });
    }
    if (summary.method) {
      cloned.push({ Name: 'Method', Value: summary.method });
    }
    if (summary.isAssetRequest) {
      cloned.push({ Name: 'AssetRequest', Value: 'true' });
    }
  ...
  if (summary.isAssetRequest && Number.isFinite(summary.durationMs)) {
    metrics.push({
      MetricName: 'AssetResponseDuration',
  ...
  if (summary.statusCode === 404) {
    metrics.push({
      MetricName: 'HttpNotFound',
      Dimensions: createDimensions(),
      Timestamp: timestamp,
      Unit: 'Count',
      Value: 1,
  ...
  if (Number.isFinite(summary.sessionCount) && summary.sessionCount > 0) {
    metrics.push({
      MetricName: 'SessionCount',
      Dimensions: createDimensions({ Detail: 'Sessions' }),
      Timestamp: timestamp,
      Unit: 'Count',
  ...
  if (Number.isFinite(summary.downloadCount) && summary.downloadCount > 0) {
    metrics.push({
      MetricName: 'DownloadCount',
      Dimensions: createDimensions({ Detail: 'Downloads' }),
      Timestamp: timestamp,
      Unit: 'Count',
...
async function publishMetrics({
  ...
  const additionalMetrics = buildAdditionalMetricsFromHttpSummary(
    httpSummary,
    dimensions,
    timestamp
  );
  ...
  if (additionalMetrics.length) {
    try {
      await cloudWatchClient.send(
        new PutMetricDataCommand({
          Namespace: metricsNamespace,
          MetricData: additionalMetrics,
        })
      );
    } catch (err) {
      console.warn('Failed to publish HTTP summary metrics', {
        functionName,
        stage: stageName,
        errorMessage: err?.message,
      });
...
export function withLambdaObservability(handler, {
  ...
  return async function observableHandler(event, context) {
    ...
    try {
      ...
      const httpSummary = isApiGatewayHttpEvent(event)
        ? describeHttpResponse(result, {
            event,
            requestMetadata,
            durationMs,
          })
      ...
      if (httpSummary) {
        const baseResponseLog = compactLogObject({
          invocationId,
          functionName,
          requestId,
          sessionId,
          ...
          operationGroup,
          durationMs,
          method: httpSummary.method,
          path: httpSummary.path,
          pathCategory: httpSummary.pathCategory,
          statusCode: httpSummary.statusCode,
          contentLength: httpSummary.contentLength,
          sessionCount: httpSummary.sessionCount,
          downloadCount: httpSummary.downloadCount,
          assetRequest: httpSummary.isAssetRequest || undefined,
        });

        console.info('API gateway response summary', baseResponseLog);

        if (httpSummary.isAssetRequest) {
          console.info('API gateway asset response', baseResponseLog);
        }

        if (httpSummary.statusCode === 404) {
          console.warn('API gateway not found response', baseResponseLog);
        }

        if (
          (Number.isFinite(httpSummary.sessionCount) && httpSummary.sessionCount > 0) ||
          (Number.isFinite(httpSummary.downloadCount) && httpSummary.downloadCount > 0)
        ) {
          console.info('API gateway session download totals', {
            ...baseResponseLog,
            sessionCount: httpSummary.sessionCount,
            downloadCount: httpSummary.downloadCount,
          });
        ...
        await appendSessionLog({
          ...
          requestId,
          jobId,
          event: 'lambda_http_response_summary',
          metadata: {
            ...baseResponseLog,
            requestMetadata,
          },
