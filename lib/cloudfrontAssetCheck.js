...
function createProxyBlockedError(url, response) {
  ...
  const error = new Error(
    `Access to ${url.toString()} is blocked by an upstream proxy (status ${response?.status || 'unknown'})${hint || ''
    }.`
  );
...
function isProxyNetworkError(error) {
  if (error?.cause?.code && typeof error.cause.code === 'string' && PROXY_NETWORK_ERROR_CODES.has(error.cause.code.trim().toUpperCase())) {
    return true;
  }
...
async function verifyAsset(fetchImpl, url, { userAgent } = {}) {
  ...
  if (CACHE_BUST_STATUS_CODES.has(finalResponse.status) && !isCloudfrontForbiddenResponse(finalResponse)) {
    ...
    recoverableIssues.push(
      `GET request to ${url.toString()} returned ${finalResponse.status} ${finalResponse.statusText || ''
  ...
  if (!finalResponse.ok) {
    const error = new Error(
      `Asset request to ${url.toString()} returned ${finalResponse.status} ${finalResponse.statusText || ''
        }`.trim()
    );
