...
async function verifyAsset(fetchImpl, url) {
  ...
  try {
    response = await fetchImpl(url.toString(), {
      method: 'HEAD',
      headers: buildRequestHeaders({ method: 'HEAD', noCache: true, origin: originHeader }),
    });
  } catch (error) {
  ...
  const fetchWithNoCache = async (targetUrl) => {
    ...
    try {
      assetResponse = await fetchImpl(targetUrl, {
        method: 'GET',
        headers: buildRequestHeaders({ method: 'GET', noCache: true, origin: originHeader }),
      });
    } catch (error) {
      if (isProxyNetworkError(error)) {
        throw createProxyBlockedNetworkError(url, error);
  ...
  const isRecoverableHeadError = [403, 404, 405, 500, 501].includes(initialStatus);
  ...
  if ([403, 404].includes(finalResponse.status)) {
...
async function verifyAssetWithPrefixes({
  ...
  const prefixes = expandAssetPathPrefixes(Array.isArray(assetPathPrefixes) ? assetPathPrefixes : []);
  ...
  for (const prefix of prefixes) {
    ...
    try {
      const result = await verifyAsset(fetchImpl, assetUrl);
      return result;
    } catch (error) {
      ...
      if (
        error?.code === CLOUDFRONT_FORBIDDEN_ERROR_CODE ||
        !RECOVERABLE_PREFIX_ERROR_STATUSES.has(status)
      ) {
        throw error;
      }
...
async function verifyOnce({ baseUrl, fetchImpl, assetPathPrefixes }) {
  ...
  try {
    response = await fetchImpl(cacheBustUrl.toString(), {
      method: 'GET',
      headers: buildRequestHeaders({ method: 'GET', noCache: true, acceptHtml: true }),
    });
  } catch (error) {
    throw new Error(
