...
async function verifyAsset(fetchImpl, url) {
  ...
  if (response.status === 404) {
    throw new Error(`Asset returned 404 at ${url.toString()}`);
  }
...
async function verifyOnce({ baseUrl, fetchImpl }) {
  ...
  let response;
  try {
    response = await fetchImpl(indexUrl.toString(), {
      method: 'GET',
      headers: { Accept: 'text/html' },
    });
  } catch (error) {
    throw new Error(`Failed to load index.html at ${indexUrl.toString()}: ${error?.message || error}`);
  }
