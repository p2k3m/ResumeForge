...
function buildRequestHeaders({ method = 'GET', noCache = false, acceptHtml = false } = {}) {
  const userAgent =
...
async function verifyAsset(fetchImpl, url) {
  ...
  try {
    response = await fetchImpl(url.toString(), {
      method: 'HEAD',
      headers: buildRequestHeaders({ method: 'HEAD', noCache: true }),
    });
  } catch (error) {
  ...
  const fetchWithNoCache = async (targetUrl) => {
    ...
    try {
      assetResponse = await fetchImpl(targetUrl, {
        method: 'GET',
        headers: buildRequestHeaders({ method: 'GET', noCache: true }),
      });
    } catch (error) {
      if (isProxyNetworkError(error)) {
        throw createProxyBlockedNetworkError(url, error);
  ...
  if ([403, 404, 405, 501].includes(initialStatus)) {
