import { renderTemplatePdf } from './index.js';
...
export async function backstopTemplate(templateId, { logger = console } = {}) {
  if (!templateId) {
    throw new Error('Template ID is required for backstopTemplate');
  }
  const results = await backstopPdfTemplates({ templates: [templateId], logger });
  return results[0];
}
