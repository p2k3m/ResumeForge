  ...
  return {
    _handleFile(req, file, cb) {
      ...
      file.stream.on('end', async () => {
        console.log(JSON.stringify({ event: 's3_streaming_storage_end', bucket, key, chunks: chunks.length }));
        try {
          ...
          try {
            ...
            console.log(JSON.stringify({ event: 's3_streaming_storage_metadata_stripped', originalSize: body.length, newSize: sanitizedBuffer.length }));
          } catch (stripError) {
          ...
          console.log(JSON.stringify({ event: 's3_streaming_storage_uploaded', bucket, key }));
          finish(null, {
            bucket,
            key,
          ...
          console.error(JSON.stringify({ event: 's3_streaming_storage_upload_failed', error: err.message, stack: err.stack }));
          finish(toUploadStorageError(err));
        }
      });
