...
export async function stripUploadMetadata({ buffer, mimeType, originalName }) {
  ...
  try {
    if (kind === 'pdf') {
      // Optimization: Check for PDF header before attempting to parse
      // The header %PDF- should appear within the first 1024 bytes
      const headerSearchLimit = Math.min(resolvedBuffer.length, 1024);
      const headerSearchBuffer = resolvedBuffer.subarray(0, headerSearchLimit);
      if (!headerSearchBuffer.includes('%PDF-')) {
        return resolvedBuffer;
      }
