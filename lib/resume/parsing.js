...
export async function classifyResumeDocument(text = '', options = {}) {
  ...
  if (resumeScore >= 3 || (resumeScore >= 2 && nonResumeClassification?.className === 'certificate')) {
    if (nonResumeClassification && nonResumeClassification.className !== 'certificate') {
      return ensureClassificationClassName(nonResumeClassification, 'non_resume');
    }
    return { isResume: true, description: 'a professional resume', className: 'resume', confidence: 0.6 };
  }
 ...
 */
export function shouldRejectBasedOnClassification(result) {
  if (!result || typeof result !== 'object') {
    return false;
  }
  ...
  const normalizedClassName =
    typeof result.className === 'string' ? result.className.trim().toLowerCase() : '';

  if (normalizedClassName && normalizedClassName !== 'non_resume') {
    return true;
  }
  ...
  const numericConfidence =
    typeof result.confidence === 'number'
      ...
      : typeof result.confidence === 'string'
        ? Number.parseFloat(result.confidence)
        : Number.NaN;
  ...
  if (isStrongNonResumeSignal(result.description, result.reason)) {
    return true;
  }
