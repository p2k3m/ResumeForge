import { normalizeSkillListInput } from './skills.js';

/**
 * Canonical enhancement operations supported across the ResumeForge platform.
 * These values drive fan-out orchestration and allow individual microservices
 * to opt-in to just the routines they require when invoking Lambda Layers.
 */
...
/**
 * Produce a deterministic enhancement patch for a resume section. The payload is
 * CV-centric and intentionally decoupled from infrastructure dependencies so it
 * can be reused across HTTP handlers, async workers, and step functions.
 */
...
/**
 * Merge a generated patch back into the current resume text. The operation is
 * pure and leaves infrastructure or persistence concerns to the caller.
 */
...
/**
 * Normalise a caller-supplied enhancement type list so orchestrators can safely
 * fan-out work items without duplicating guard rails.
 */
...
export default generateEnhancementPatch;
